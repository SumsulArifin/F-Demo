...
class _WidgetAnimatorState extends State<WidgetRingAnimator>
  ...
  void initiateNavigation() {
    ...
    Future.delayed(Duration(seconds: 3), () {
      widget.navigateToHomeScreen(); // Call the navigation callback
    });
  }
...
class Arc2Painter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < noOfImages; i++) {
      ...
      canvas.drawImage(
        image[i],
        pointOnArc + Offset(-(image[i].width / 2).toDouble(),
      ...
      canvas.drawArc(
        rect,
        degreesToRads((i * arcAngle) + image[i].width / 2).toDouble(),
        degreesToRads(arcAngle - image[i].width).toDouble(),
        false,
        p,
      );
    }
